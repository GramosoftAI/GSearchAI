        ...
        * The one exception is the coverage-validation fallback task (task_id starts
        ...
        if self.db:
            try:
                ...
                section_filter_sql = ""
                if target_sections:
                    section_filter_sql = " AND c.section = ANY(:target_sections) "
                elif is_tabular:
                    section_filter_sql = " AND c.chunk_index >= 90000 "

                hybrid_sql = f"""
                WITH vector_candidates AS (
                    SELECT 
                        c.id, c.text, c.s3_path, c.metadata_json, 
                        c.chunk_index, kb.name as kb_name,
                ...
                SELECT 
                    vc.id AS id,
                    vc.text AS text,
                    vc.s3_path AS s3_path,
                    vc.metadata_json AS metadata_json,
                    vc.chunk_index AS chunk_index,
                    vc.kb_name AS kb_name,
                    vc.vec_score AS vec_score,
                    0.0 AS fts_score,
                    vc.section AS section,
                    vc.vec_score AS rrf_score
                ...
                if target_sections:
                    sql_params["target_sections"] = target_sections

                res = await self.db.execute(text(hybrid_sql), sql_params)
                ...
                for idx, row in enumerate(all_rows):
                    ...
                    chunks.append(RetrievedChunk(
                        ...
                        kb_id=str(kb_ids[0]),
                        position=idx,
                        embedding_similarity=vec_sim,
                        graph_score=fts_score,
                        hybrid_score=rrf,
                        reason="VECTOR_FTS_HYBRID",
                        source=row.s3_path or row.kb_name or f"DocumentChunk {row.chunk_index}",
                        s3_path=row.s3_path,
                        engine_name="vector",
                ...
                final_chunks = chunks
                latency = time.time() - trace_start
                logger.info(f"[TRACE_E2E] [EXIT] VectorEngine.retrieve - Output: {len(final_chunks)} chunks (pgvector hybrid) - Latency: {latency:.2f}s")
                return final_chunks
            except Exception as e:
